...
CHECKSUMS
/usr/bin/python3 - "$build_dir/sshfs.c" <<'PYTHON'
from pathlib import Path
import sys
p = Path(sys.argv[1])
s = p.read_text()
anchor = "\t/* Readahead should be done by kernel or sshfs but not both */"
if s.count(anchor) != 1:
    raise SystemExit("Unexpected SSHFS source; refusing to patch")
s = s.replace(anchor, "#ifdef __APPLE__\n\tconn->want_darwin &= ~FUSE_DARWIN_CAP_RENAME_EXT;\n#endif\n" + anchor)
p.write_text(s)
PYTHON
cat > "$build_dir/config.h" <<'CONFIG'
#define PACKAGE_VERSION "3.7.5-rws-fskit1"
...
printf '%s\n' "Upstream: $source_commit" "Patch: disable unsupported FUSE_DARWIN_CAP_RENAME_EXT" > "$build_dir/BUILD-INFO.txt"
