...
The subsequent Omarchy read-only check found SSH healthy at 0.09 seconds but the original volume absent, with a stuck macOS FSKit mount subprocess, orphan SSHFS process, blocked Git I/O, and a launchd run still active after 12 hours. The branch adds an embedded Arch NFS server provisioner with SSH staging, Tailscale-only binding, strict preflight and a guarded remove command. Both hosts passed CLI setup `--dry-run`; the script passed `bash -n` and the new Rust unit tests passed. Provisioning has not run on Omarchy or any production path. The changed app UI has not passed a full Swift build because the current Command Line Tools cannot load `SwiftUIMacros`; this is a build-tool limitation, not app runtime validation. The two original paths and reboot acceptance remain open.
