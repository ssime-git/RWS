...
Read-only production check later on 24 September: the loaded `io.rws.mounts` job now has `StartInterval=30`, but launchd reports one invocation still `running` (PID 3449, 25 runs, last exit 0). Its error log was last modified at 11:18; the job remained active roughly 11 hours later. The installed RWS status reports Omarchy disconnected and Razer connected; an Omarchy SSHFS process remains present despite no Omarchy mount entry. Both SSH connections completed in 0.16 s (Omarchy) and 0.10 s (Razer) with `BatchMode=yes`; current SSH connection latency does not explain the missing mount. The installed error log records prior 5-second SSHFS start timeouts and an unresponsive Omarchy mount, but has no recent trace of the current job. The present `rws` process could not be sampled without elevated debugger permission, so its exact blocked syscall is unknown. The local `launchd.plist(5)` manual states that a `StartInterval` firing is missed while the job is running; the 30-second setting alone cannot recover this stuck invocation. No process, service, or volume was interrupted during this check.

The isolated branch now changes `autostart run` into a supervisor. It launches one canonical `autostart run-one NAME` child per workspace in parallel, bounds each child to 90 seconds and terminates its process group on timeout; paused intent is re-read by the child. A regression test confirms a paused child does no mount work or config write, and the existing process tests cover bounded timeout and descendant termination. This prevents one workspace's stalled syscall from keeping the shared LaunchAgent invocation alive indefinitely or postponing the other workspace in a future installation. It cannot force an uninterruptible kernel task to finish, clear FSKit's stale mount table, or guarantee that an already wedged volume remounts. The production CLI/LaunchAgent were not replaced or restarted as part of this branch validation.

A later **real Razer** kernel-server trial changed the NFS feasibility result. An isolated Tailscale-only `/tmp` export allowed native macOS NFSv4.0 with `nocallback` to pass bidirectional file I/O, symlinks, chmod, Unicode NFC/NFD lookup, atomic replace, `fsync`, POSIX lock contention/release, and a normal unmount/remount cycle. NFSv4.1 file I/O needed `nocallback` and then a contended lock caused `close` to block; the prototype now selects v4.0. Linux-local `flock` did not coordinate with Mac `flock`, while cross-host POSIX locks did. With a disposable configuration, the branch CLI connected and attested `/Volumes/RWS-razer-trial`, wrote and verified a file remotely, passed `doctor`, handled a repeat connect, and disconnected normally; `/Volumes/RWS-razer-1` remained mounted via FSKit. The user ran the trial cleanup; read-only checks confirmed the NFS service inactive and both temporary configuration files absent. `nfs-utils` and trial files under `/tmp` remain. This does **not** validate the two original paths, Omarchy, the packaged app, network-loss recovery, or a reboot.

The subsequent Omarchy read-only check found SSH healthy at 0.09 seconds but the original volume absent, with a stuck macOS FSKit mount subprocess, orphan SSHFS process, blocked Git I/O, and a launchd run still active after 12 hours. The branch adds an embedded Arch NFS server provisioner with SSH staging, Tailscale-only binding, strict preflight and a guarded remove command. Both hosts passed CLI setup `--dry-run`; the script passed `bash -n` and the new Rust unit tests passed. Provisioning has not run on Omarchy or any production path. The changed app UI builds successfully with `SDKROOT` set to the installed macOS 26.5 SDK; the default 27.0 SDK still fails on missing `SwiftUIMacros`. The packaged app has not been installed or exercised. The two original paths and reboot acceptance remain open.

After the user authorized a targeted stop of Omarchy-related processes, the four read-only Delta Git subprocesses and orphan Omarchy SSHFS process received SIGTERM. None exited. SIGKILL then removed SSHFS, but the Git processes remained in macOS `U` (uninterruptible I/O) state. The Omarchy-only `/sbin/mount` subprocess exited after SIGTERM, along with its `sudo` parent. The global macFUSE helper, Delta parent process, launchd job, and Razer SSHFS were not stopped. Razer remained mounted; Omarchy remained absent. This targeted cleanup does not clear the blocked Git I/O or establish FSKit recovery.

The user subsequently authorized stopping the Delta parent and frozen RWS launchd run. Delta received SIGTERM/SIGKILL but remained in macOS `?E`; the four Git children remained in `U`. `launchctl bootout` returned success, but launchd still displayed the RWS job as `SIGKILLed` with its blocked PID. Razer's mounted FSKit volume and SSHFS process remained active. The changed app passed `SDKROOT=/Library/Developer/CommandLineTools/SDKs/MacOSX26.5.sdk swift build --package-path macos -j 2`; this does not validate app runtime or reboot recovery.

Omarchy then passed a real isolated kernel NFSv4.0 trial on `/tmp`: bidirectional file operations, fsync, chmod, symlinks, Unicode aliasing, atomic replace, local locks, cross-host POSIX contention/release (0.078-second Mac close), branch CLI mount/attestation/proof/idempotence/disconnect at `/Volumes/RWS-ssime-omarchy-trial`. A managed systemd drop-in for `tailscale-online.target` and an IP assertion started successfully and was removed after an isolated setup. Two temporary NFS exports on Razer and Omarchy then mounted concurrently; Razer attested at first attempt, while Omarchy's initial proof read timed out after 12 seconds and immediately passed `--verify-existing` with both mounted. Concurrent writes on both volumes were verified from the remote hosts, and both volumes disconnected normally. A narrowly scoped one-time retry for that initial NFS proof-read timeout was added with a unit test. Both temporary exports were then removed through the CLI; follow-up found `nfs-server` inactive/disabled on each host with no RWS managed configuration files. Production paths, app install, delayed Tailscale startup, and reboot recovery are not yet validated.
